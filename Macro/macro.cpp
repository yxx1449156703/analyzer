...
void Macro::on_pushButton_clicked()
{
...
void Macro::on_pushButton_2_clicked()
{
    st.clear();ssr.clear();sr.clear();k.clear();kk.clear();
    ...
    tet.close();
    QFile tx("code.txt");
    tx.open(QFile::WriteOnly|QFile::Truncate);
    tx.close();
    QFile t("grammer.txt");
    t.open(QFile::WriteOnly|QFile::Truncate);
    t.close();
    QFile x("signtable.txt.txt");
    x.open(QFile::WriteOnly|QFile::Truncate);
    x.close();
    temp=1;
    label=0;
    ...
    if ((fp=fopen("code.txt","r"))==NULL)				//读入文件
        ...
        QString dd=in5.readAll();
        for(int i=0;i<ssr.length();i++){
               dd.replace(sr[i],ssr[i]);
                }
        for(int i=0;i<k.length();i++){
               dd.replace(k[i],kk[i]);
                }
        ui->textEdit_3->setPlainText(dd);
        file1.flush();
        file1.close();
        if(j==1)
        {
        ...
        else
        {
            ...
            QObject::connect(this,SIGNAL(sendData(QString)),warning,SLOT(receiveData(QString)));
            emit sendData(da);
...
void Macro::on_pushButton_3_clicked()
{
    QString path=QFileDialog::getOpenFileName(this,tr("Open File"),".",tr("Text Files(*.txt)"));
}
...
int S()									//语义分析
{
    ...
    if(strcmp(ech[now],"if")==0)		//now=0，字符是if
    {
        ...
        now++;
        ofstream outfile1("grammertable.txt", ios::app);
        outfile1 << "  S->if C then S"<< endl;
        c=C();
        if(c==0)
        {
            return 0;
...
int SS()                                  //SS
{
    ...
    if (strcmp(ech[now], ";") == 0) {		//？
        ...
        strcpy(Ctrue, Snext);
        printf(" AAAAAAAAAAAA%sAAAAAAAA",ech[now + 1]);
        if ((strcmp(ech[now + 1], "") != 0)&&(strcmp(ech[now + 1], "") != 0)){
            printf("\n%s:\n", Snext);
            ofstream outfile("3address.txt",ios::app);
            outfile<<Snext<<":"<<endl;
            ofstream outfile1("grammertable.txt", ios::app);
